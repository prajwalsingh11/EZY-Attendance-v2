...
window = tk.Tk()
window.title("EZY Attendance Using Face Recognition")

window.geometry('1280x800')
window.configure(background='snow')

bg_image = tk.PhotoImage(file="banner4.png")  # Replace with your image path
...
def send_mail():
    ...
    sender_email = "kishnuyadav783@gmail.com"  # Enter your email
    sender_password = ""  # Enter your password
    receiver_email = "21bcar0376@jainuniversity.com"  # Enter recipient email
    subject = "Attendance CSV Files"
    ...
    # Message body
    body = "Please find the attendance CSV files attached."
    msg.attach(MIMEText(body, 'plain'))

    # Attach CSV files from the Attendance folder
    folder_path = "Attendance"
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv"):
            attachment = open(os.path.join(folder_path, filename), "rb")
    ...
    # Call the send_mail function when the button is clicked


sendMailBtn = tk.Button(window, text="Send Mail", command=send_mail, fg="black", bg="green", width=15, height=2,
                        font=('times', 15, ' bold '))
sendMailBtn.place(x=90, y=600)

def show_developers():
    # Add code to display information about developers
    os.system('python3 developer.py')
...
def manually_fill():
    ...
    def fill_attendance():
        ...
        try:
            global cursor
            connection = pymysql.connect(host='localhost', user='root', password='61366136', db='manually_fill_attendance')
        ...
        if subb=='':
            ...
            def create_csv():
                ...
                csv_name='/home/krishna/project-biometric/modify/Attendance/Manually Attendance/'+DB_table_name+'.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(r'explorer /select,"/home/krishna/project-biometric/modify/Attendance/Manually Attendance/-------Check atttendance-------"')
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                try:
                    global cursor
                    connection = pymysql.connect(host='localhost', user='root', password='61366136', db='Face_reco_fill')
                    cursor = connection.cursor()
                except Exception as e:
                    print(e)
                ...
                root.configure(background='snow')
                cs = '/home/krishna/project-biometric/modify/' + fileName
    ...
    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"/home/krishna/project-biometric/modify/Attendance/-------Check atttendance-------"')
    ...
    def log_in():
        ...
        if username == 'krishna' :
            if password == 'krishna123':
                ...
                cs = '/home/krishna/project-biometric/modify/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
...
background_image = PhotoImage(file="Images_GUI/bannerezy.png")

# Create the label with the background image
message = tk.Label(window, text="", bg="black", fg="white", width=1000,
                   height=140, font=('times', 32, 'bold '), borderwidth=2, relief="groove", image=background_image, compound="center")

# Ensure the image is not garbage collected
message.image = background_image

message.place(relx=0.5, y=80, anchor=tk.CENTER)

Notification = tk.Label(window, text="All things good", bg="Green", fg="white", width=15,
                      height=3, font=('times', 17, 'bold'))

lbl = tk.Label(window, text="Enter USN", width=20, height=2, fg="white", bg="black", font=('times', 15, ' bold '))
...
clearButton = tk.Button(window, text="Clear",command=clear,fg="red"  ,bg="lime"  ,width=10  ,height=1 ,activebackground = "Red" ,font=('times', 15, ' bold '))
clearButton.place(x=950, y=210)

clearButton1 = tk.Button(window, text="Clear",command=clear1,fg="red"  ,bg="lime"  ,width=10 ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
clearButton1.place(x=950, y=310)

AP = tk.Button(window, text="Check Register students",command=admin_panel,fg="black"  ,bg="white"  ,width=25 ,height=1, activebackground = "lime" ,font=('times', 15, ' bold '))
AP.place(x=990, y=410)

takeImg = tk.Button(window, text="Take Images",command=take_img,fg="white"  ,bg="green"  ,width=20  ,height=3, activebackground = "lime" ,font=('times', 15, ' bold '))
takeImg.place(x=90, y=500)

trainImg = tk.Button(window, text="Train Images",fg="white",command=trainimg ,bg="black"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
trainImg.place(x=390, y=500)

FA = tk.Button(window, text="Automatic Attendace",fg="black",command=subjectchoose  ,bg="white"  ,width=20  ,height=3, activebackground = "lime" ,font=('times', 15, ' bold '))
FA.place(x=690, y=500)

quitWindow = tk.Button(window, text="Manual Attendance", command=manually_fill  ,fg="black"  ,bg="lawn green"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
quitWindow.place(x=990, y=500)

sendMailBtn = tk.Button(window, text="Send Mail", command=send_mail, fg="black", bg="white", width=15, height=2, font=('times', 15, ' bold '))
sendMailBtn.place(x=90, y=600)

developersBtn = tk.Button(window, text="Developers", command=show_developers, fg="green", bg="white", width=15, height=2, font=('times', 15, ' bold '))
developersBtn.place(x=290, y=600)

helpBtn = tk.Button(window, text="Help", command=show_help, fg="red", bg="black", width=15, height=2, font=('times', 15, ' bold '))
helpBtn.place(x=490, y=600)
